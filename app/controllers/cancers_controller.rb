...
class CancersController < ApplicationController
  ...
  def show
    ...
    @cut_off = 2;
    @mutties = @consensus_cancer_gene.mutations.order('nuc_position1').where("mutation_counter > ?", @cut_off)
...
#increaes dyno count---- 50
  ...
  def import
    if params[:file]
      ...
      make_jtable = CancerImporter.new
      make_jtable.mutations_join_table
